...
export default function Doctors() {
  ...
  function resetForm() {
    setForm({
      ...
      specialization: '', qualification: '', experience: '', status: 'active',
  ...
  async function fetchDoctors() {
    try {
      ...
      if (search) {
        const like = `%${search}%`
        query = query.or(`name.ilike.${like},email.ilike.${like},phone.ilike.${like},specialization.ilike.${like}`)
      }
  ...
  async function handleSave(e) {
    ...
    try {
      ...
      if (editing) {
        ...
        // Create doctor via Edge Function (requires deployment)
        if (!createLogin) throw new Error('Creating a doctor requires a login. Please enable "Create login".')
  ...
  function exportCSV() {
    const headers = ['name','email','mobile','specialization','qualification','experience','status']
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <input value={search} onChange={e=>{setSearch(e.target.value); setPage(1)}} placeholder="Search name / email / mobile / specialization" className="border rounded px-3 py-1.5 text-sm w-72" />
          ...
          <button onClick={openCreate} className="bg-blue-600 text-white px-3 py-2 rounded text-sm hover:bg-blue-700">Add Doctor</button>
        </div>
      </div>
          ...
          <tbody>
            {rows.map(r => (
              <tr key={r.id} className="border-t">
                ...
                <td className="px-4 py-2">{r.qualification || '-'}</td>
            ...
            {rows.length===0 && (
              <tr><td colSpan={8} className="px-4 py-10 text-center text-gray-500">{loading? 'Loading…' : 'No doctors found'}</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      {pwInfo && (
        <div className="fixed bottom-4 right-4 z-50 bg-white border rounded shadow-lg p-3 w-[320px]">
          <div className="font-semibold mb-1">Doctor login created</div>
            ...
            <form onSubmit={handleSave} className="p-5 space-y-3">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <Field label="Specialization">
                  <input value={form.specialization} onChange={e=>setForm({...form, specialization:e.target.value})} className="w-full border rounded px-3 py-2" placeholder="e.g., Dentist, Orthodontist" />
                </Field>
                <Field label="Qualification">
                  <input value={form.qualification} onChange={e=>setForm({...form, qualification:e.target.value})} className="w-full border rounded px-3 py-2" placeholder="e.g., BDS, MDS" />
                </Field>
                <Field label="Experience">
                  <input value={form.experience} onChange={e=>setForm({...form, experience:e.target.value})} className="w-full border rounded px-3 py-2" placeholder="e.g., 5 years" />
                </Field>
                <Field label="Status">
              ...
              {!editing && (
                <div className="space-y-2 border-t pt-3">
                  <label className="flex items-center gap-2 text-sm">
                    ...
                    <span>Create login for doctor</span>
                  </label>
                  {createLogin && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <Field label="Send authentication email">
                        ...
                        <p className="text-xs text-gray-500 mt-1">Select "No" to skip email verification.</p>
                      </Field>
                      <Field label="Default password (optional)">
                        ...
                        <p className="text-xs text-gray-500 mt-1">Doctor can use this password to login directly without email verification.</p>
                      </Field>
                    </div>
                  )}
                </div>
